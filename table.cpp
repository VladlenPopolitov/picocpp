...
int Picoc::TableSet(struct Table *Tbl, char *Key, struct Value *Val, const char *DeclFileName, int DeclLine, int DeclColumn)
{
	Picoc *pc = this;
    int AddAt;
    ...
    if (FoundEntry == NULL)
    {   /* add it to the table */
		struct TableEntry *NewEntry = static_cast<TableEntry*>(VariableAlloc( nullptr, sizeof(struct TableEntry), Tbl->OnHeap));
...
char *Picoc::TableSetIdentifier( struct Table *Tbl, const char *Ident, int IdentLen)
{
	Picoc *pc = this;
    int AddAt;
    ...
    {   /* add it to the table - we economise by not allocating the whole structure here */
		struct TableEntry *NewEntry = static_cast<TableEntry*>(HeapAllocMem( sizeof(struct TableEntry) - 
...
char *Picoc::TableStrRegister2( const char *Str, int Len)
{
	Picoc *pc = this;
    return TableSetIdentifier( &pc->StringTable, Str, Len);
}

char *Picoc::TableStrRegister( const char *Str)
{
